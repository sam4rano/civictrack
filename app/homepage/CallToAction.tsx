  ...
  return (
    <div className="py-20 ">
      <div className="flex flex-col gap-8">
        <h1 className="text-center text-3xl sm:text-xl sm:font-bold font-kanit text-pink-600">Our Comprehensive database</h1>
        <div className="flex flex-row sm:flex-col gap-10 justify-center align-middle items-center mx-auto ">
          {todoData.map((todo) => (
            <ul key={todo.id} className="flex flex-col justify-between items-center gap-2 w-full sm:bg-slate-400 p-2">
              <p className="text-2xl font-bold text-center font-kanit">{todo.count}</p>
              <h1 className="text-xl text-center font-montserrat">{todo.name}</h1>
            </ul>
          ))}
        </div>
      </div>
